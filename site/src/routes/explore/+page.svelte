<script>
  ...
  import { comparable } from '$lib/hw.js';
  ...
  let hwCompare = $derived([...comparable(data.heatwaves?.stations ?? [])].sort((a, b) => a.threshold_f - b.threshold_f));

  let closed = $derived(allStations.length - stations.length);
</script>

<svelte:head>
  <title>Explore Los Angeles heat, night by night, since 1893 · climate.sorkinlabs</title>
  <meta property="og:title" content="Explore: Los Angeles nights aren't cooling off like they used to" />
  <meta property="og:description" content="Hot days, warm nights and frost at {stations.length} LA-area weather stations, every year, from NOAA's hourly station records." />
</svelte:head>

<p class="eyebrow">Explorer · everything behind the <a href="/">front-page story</a></p>
<section class="hero" id="nights">
  <div class="text">
...
{#if hwCompare.length}
  <section id="heatwaves">
    <h2>Heat waves per summer, year by year</h2>
    <p class="muted">The <a href="/">front-page</a> heat waves, counted per complete summer at each station with a baseline. The count swings from year to year and decade to decade — the 2000s were quiet everywhere, the 1950s busy at the coast — but at the coast and in the basin the recent average sits where the mid-century one did; inland stations run about one extra heat wave a year.</p>
